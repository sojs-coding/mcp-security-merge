...
def register_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    async def illusive_networks_list_deceptive_items(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], deceptive_type: Annotated[List[str], Field(..., description="Specify what kind of deceptive items should be returned.")], deceptive_state: Annotated[List[str], Field(..., description="Specify what kind of deceptive items should be returned based on state.")], max_items_to_return: Annotated[str, Field(default=None, description="Specify how many items to return. Default: 50. If nothing is specified, action will return all items.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
