    ...
    async def varonis_data_security_platform_update_alert(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], alert_guid: Annotated[str, Field(..., description="Specify alert GUID for the update. This action can run on multiple alerts. Multiple alerts can be specified as a comma-separated string.")], alert_status: Annotated[List[Any], Field(..., description="Specify the alert status to update to.")], closing_reason: Annotated[Optional[List[Any]], Field(default=None, description="Specify the closing reason for the alert. When the alert status is changed to \"closed\", closing reason must be specified.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        try:
            instance_response = await bindings.http_client.get(
                Endpoints.LIST_INTEGRATION_INSTANCES.format(INTEGRATION_NAME="VaronisDataSecurityPlatform")
            )
            instances = instance_response.get("integration_instances", [])
        except Exception as e:
            # Log error appropriately in real code
            print(f"Error fetching instance for VaronisDataSecurityPlatform: {e}")
        ...
        if instances:
            ...
            script_params = {}
            script_params["Alert GUID"] = alert_guid
            script_params["Alert Status"] = alert_status
            ...
            action_data = ApiManualActionDataModel(
                ...
                actionName="VaronisDataSecurityPlatform_Update Alert",
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "VaronisDataSecurityPlatform_Update Alert", # Assuming same as actionName
            ...
            try:
                ...
                print(f"Error executing action VaronisDataSecurityPlatform_Update Alert for VaronisDataSecurityPlatform: {e}")
                return {"Status": "Failed", "Message": f"Error executing action: {e}"}
        else:
            print(f"Warning: No active integration instance found for VaronisDataSecurityPlatform")
    ...
    async def varonis_data_security_platform_ping(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                isPredefinedScope=is_predefined_scope, # Pass the is_predefined_scope parameter
                actionProvider="Scripts", # Assuming constant based on example
                actionName="VaronisDataSecurityPlatform_Ping",
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "VaronisDataSecurityPlatform_Ping", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
            ...
            try:
                ...
                # Log error appropriately
                print(f"Error executing action VaronisDataSecurityPlatform_Ping for VaronisDataSecurityPlatform: {e}")
                return {"Status": "Failed", "Message": f"Error executing action: {e}"}
        else:
            print(f"Warning: No active integration instance found for VaronisDataSecurityPlatform")
            return {"Status": "Failed", "Message": "No active instance found."}
