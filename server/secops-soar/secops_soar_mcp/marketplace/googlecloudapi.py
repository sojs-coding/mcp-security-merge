...
def register_tools(mcp: FastMCP):
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    ...
                    "ScriptName": "GoogleCloudApi_Ping", # Assuming same as actionName
    ...
    @mcp.tool()
    async def google_cloud_api_execute_http_request(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], method: Annotated[List[Any], Field(..., description="Specify the method for the request.")], url_path: Annotated[str, Field(..., description="Specify the URL that needs to be executed.")], fields_to_return: Annotated[str, Field(..., description="Specify what fields to return. Possible values: response_data, redirects, response_code,response_cookies,response_headers,apparent_encoding")], request_timeout: Annotated[str, Field(..., description="How long to wait for the server to send data before giving up")], url_params: Annotated[Optional[str], Field(default=None, description="Specify the parameters for the URL. Any value provided in this parameter will be used alongside the values that are directly provided in the URL path parameters.")], headers: Annotated[Optional[str], Field(default=None, description="Specify headers for the HTTP request.")], cookie: Annotated[Optional[str], Field(default=None, description="Specify the parameters that should be constructed into the \"Cookie\" header. This parameter will overwrite the cookie provided in the \"Headers\" parameter.")], body_payload: Annotated[Optional[str], Field(default=None, description="Specify body for the HTTP request.")], expected_response_values: Annotated[Optional[str], Field(default=None, description="Specify the expected response values. If this parameter is not empty, then action will work in ASYNC mode and action will execute until the expected values will be seen or until timeout.")], follow_redirects: Annotated[Optional[bool], Field(default=None, description="If enabled, action will follow the redirects.")], fail_on_4xx_5xx: Annotated[Optional[bool], Field(default=None, description="If enabled, action will fail, if the status code of the response is 4xx or 5xx.")], base64_output: Annotated[Optional[bool], Field(default=None, description="If enabled, action will convert the response to base64. This is useful when downloading files. Note: JSON result can't be bigger than 15 mb.")], save_to_case_wall: Annotated[Optional[bool], Field(default=None, description="If enabled, action will save the file and attach it to the case wall. Note: the file will be archived with \".zip\" extension. This zip will not be password protected.")], password_protect_zip: Annotated[Optional[bool], Field(default=None, description="If enabled, action will add an \"infected\" password to the zip created with \"Save To Case Wall\" parameter. Use this, when you are dealing with suspicious files.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "GoogleCloudApi_Execute HTTP Request", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
