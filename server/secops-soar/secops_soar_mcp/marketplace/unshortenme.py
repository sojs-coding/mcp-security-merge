...
def register_tools(mcp: FastMCP):
    # This function registers all tools (actions) for the UnshortenMe integration.
    ...
    async def unshorten_me_unshorten_url(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        try:
            instance_response = await bindings.http_client.get(
                Endpoints.LIST_INTEGRATION_INSTANCES.format(INTEGRATION_NAME="UnshortenMe")
            )
            instances = instance_response.get("integration_instances", [])
        except Exception as e:
            # Log error appropriately in real code
            print(f"Error fetching instance for UnshortenMe: {e}")
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    ...
                    "ScriptName": "UnshortenMe_Unshorten URL", # Assuming same as actionName
            ...
            try:
                ...
                print(f"Error executing action UnshortenMe_Unshorten URL for UnshortenMe: {e}")
                return {"Status": "Failed", "Message": f"Error executing action: {e}"}
        else:
            print(f"Warning: No active integration instance found for UnshortenMe")
    ...
    async def unshorten_me_ping(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "UnshortenMe_Ping", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
            ...
            try:
                ...
                # Log error appropriately
                print(f"Error executing action UnshortenMe_Ping for UnshortenMe: {e}")
                return {"Status": "Failed", "Message": f"Error executing action: {e}"}
        else:
            print(f"Warning: No active integration instance found for UnshortenMe")
            return {"Status": "Failed", "Message": "No active instance found."}
