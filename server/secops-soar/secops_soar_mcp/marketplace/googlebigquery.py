...
def register_tools(mcp: FastMCP):
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    ...
                    "ScriptName": "GoogleBigQuery_Run Custom Query", # Assuming same as actionName
    ...
    async def google_big_query_run_sql_query(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], dataset_name: Annotated[str, Field(..., description="Specify the name of the dataset, which will be used, when executing queries.")], query: Annotated[str, Field(..., description="Specify the SQL query that needs to be executed.")], max_results_to_return: Annotated[Optional[str], Field(default=None, description="Specify how many results to return in the response.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "GoogleBigQuery_Run SQL Query", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
    ...
    @mcp.tool()
    async def google_big_query_ping(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "GoogleBigQuery_Ping", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
