...
def register_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    async def fire_eye_nx_add_ips_policy_exception(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], victim_ip_subnet: Annotated[str, Field(..., description="Specify the IP subnet of the victim that should be used to create a new policy exception. Format: x.x.x.x/xx Example: 10.0.0.1/24")], interface: Annotated[List[Any], Field(..., description="Specify what interface should be used in policy exceptions.")], mode: Annotated[List[Any], Field(..., description="Specify the mode that should be used in the policy exception.")], name: Annotated[Optional[str], Field(default=None, description="Specify the name for the policy exception. If nothing is specified, action will add policy exception with name Siemplify_{Interface}_{Mode}")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    ...
                    "ScriptName": "FireEyeNX_Add IPS Policy Exception", # Assuming same as actionName
    ...
    @mcp.tool()
    async def fire_eye_nx_ping(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "FireEyeNX_Ping", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
    ...
    async def fire_eye_nx_download_alert_artifacts(case_id: Annotated[str, Field(..., description="The ID of the case.")], alert_group_identifiers: Annotated[List[str], Field(..., description="Identifiers for the alert groups.")], alert_uuid: Annotated[str, Field(..., description="Specify the alert uuid from where we need to download artifacts.")], download_path: Annotated[str, Field(..., description="Specify where the action should save the files.")], target_entities: Annotated[List[TargetEntity], Field(default_factory=list, description="Optional list of specific target entities (Identifier, EntityType) to run the action on.")], scope: Annotated[str, Field(default="All entities", description="Defines the scope for the action.")]) -> dict:
        ...
        if instances:
            ...
            action_data = ApiManualActionDataModel(
                ...
                properties={
                    "IntegrationInstance": instance_identifier,
                    "ScriptName": "FireEyeNX_Download Alert Artifacts", # Assuming same as actionName
                    "ScriptParametersEntityFields": json.dumps(script_params)
                }
            )
